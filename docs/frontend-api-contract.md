...
The v1 client offers source and readable Markdown modes without interpreting inline Markdown. Its readable mode recognizes only block headings, lists, quotations, paragraphs, and fenced code; every file-derived value is inserted as a text child. Raw HTML, links, images, and embeds therefore remain visible text and never become elements, attributes, or requests. Shiki code tokens are likewise rendered as text children. HTML has rendered and source tabs; only the rendered endpoint enters an empty-sandbox iframe. Preview source and zoom remain in memory, while browser history records only validated virtual paths and side/full-page display mode, never file contents.
