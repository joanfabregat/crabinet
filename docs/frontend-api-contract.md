...
The v1 client offers source and readable Markdown modes without interpreting inline Markdown. Its readable mode recognizes only block headings, lists, quotations, paragraphs, and fenced code; every file-derived value is inserted as a text child. Raw HTML, links, images, and embeds therefore remain visible text and never become elements, attributes, or requests. HTML source is embedded only from the dedicated endpoint in an iframe whose `sandbox` attribute is present and empty. Preview source and view state remain in memory and browser history records only validated virtual paths, never file contents.
