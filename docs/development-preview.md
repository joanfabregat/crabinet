...
The routed preview at <https://files-dev.jf.ffwip.com/> runs only while an agent holds the foreground process started by `scripts/dev-preview.sh`. The repo-local launcher uses rootless Podman inside a bounded transient cgroup, a read-only checkout mount, loopback-only port publishing, and the same GCE metadata denial as the task runner. Vite and the debug Rust binary run in one container with synthetic shares, generated configuration, and session state in tmpfs. No persistent service, database, or named Podman network is created. When owner-only Google client and email mapping files exist under `~/.local/state/crabinet-preview/oidc`, the launcher mounts them read-only and enables OIDC for the synthetic preview. Both mapped Google users can edit the synthetic Working files share, which opens first so create and upload buttons are visible. Reference library remains read-only.
