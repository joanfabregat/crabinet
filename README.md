...
- Folder-first browsing, metadata, conditional and ranged downloads, create/rename/move/delete operations, UTF-8 text editing, and streaming multipart uploads.
- Bounded code and text previews, safe Markdown rendering without raw HTML, and HTML shown only as inert source in an empty-sandbox iframe. Uploaded HTML cannot execute scripts, submit forms, or make network requests.
